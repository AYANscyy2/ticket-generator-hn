...
function App() {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const shareableLink = `${window.location.origin}/ticket?name=${encodeURIComponent(name)}&team=${encodeURIComponent(team)}&theme=${theme}`;
      ...
      canvas.toBlob(async (blob) => {
        const file = new File([blob], 'hacknitr-ticket.png', { type: 'image/png' });
        ...
        if (navigator.share && navigator.canShare({ files: [file] })) {
          ...
          downloadImage(canvas);
          copyLinkToClipboard(shareableLink);
          alert('📱 Image downloaded and link copied to clipboard! Upload it to Instagram Story from your phone.');
        }
        setIsGenerating(false);
      }, 'image/png');
    } catch (error) {
